...
現時点の v0 は線形フローに限定されている。 citeturn42view0turn10view0turn10view1turn10view2
...
Petri Net と Workflow Net は、構文記述を越えた健全性、到達性、検証可能性の足場になる。 citeturn40view0turn40view1turn36view1turn35view0turn35view1turn34view3turn34view4turn17search18
...
README では、「既存理論で裏づけ可能な部分」と「repo 独自仮説」の境界を明記する必要がある。 citeturn10view0turn10view1turn10view2turn36view1turn16search17
...
repo に登録する説明としては、説明層に BPMN、EventStorming、RACI を置き、意味論層に typed FP、Design by Contract、DDD を置き、検証層に Petri Net、Workflow Net、LTS を置く三層構成が安定する。 citeturn34view0turn37view0turn39view4turn35view0turn35view1turn34view3turn34view4turn36view5
...
この方針によって、内部モデルから特別記号をできるだけ排除している。 citeturn42view0turn10view0
...
この設計は、通常の CRUD 記述や単純な event-log 記述よりも、責任、観測、公開可能性に重心を置く。 citeturn10view1turn10view0
...
現時点の v0 は線形フローのみを対象にし、分岐、合流、並列は将来の graph quotient projection に委ねている。 citeturn10view2
...
理論説明では、「これは BPMN 代替 runtime ではなく、Activity 中心の semantic core である」と明示すると誤読を減らせる。 citeturn10view2turn42view0
...
repo へ入れる引用候補は、`Marlow et al., Haskell 2010 Language Report`、`Backus 1978`、`Hughes 1989` である。 citeturn42view0turn10view0turn40view0turn14search2turn14search0turn28search1turn28search19
...
repo 引用候補は、`Eilenberg and Mac Lane 1945`、`Mac Lane 1978`、`Fong and Spivak 2019` である。 citeturn36view1turn40view2turn40view3turn40view1turn37view1
...
repo 引用候補は、`Hoare 1969`、`Meyer 1992`、`Cok et al. / OpenJML` である。 citeturn35view0turn35view1turn36view6turn37view2turn27search9
...
repo 引用候補は、`Evans 2003/2015`、`Fowler 2014 Bounded Context`、`Kapferer et al. 2020 Context Mapper` である。 citeturn34view3turn40view5turn13search21turn37view1turn27search8turn37view3
...
repo 引用候補は、`Hewitt, Bishop, Steiger 1973`、`Hoare 1985 / 1984`、`Milner 1989` である。 citeturn35view2turn16search17turn16search14turn38view2turn23search10turn38view3
...
repo 引用候補は、`Harel 1987`、`W3C SCXML 2015`、`itemis CREATE docs` である。 citeturn10view0turn16search6turn34view7turn38view3turn38view4
...
repo 引用候補は、`Fowler 2005`、`Brandolini 2013- / official book`、`Overeem et al. 2021 lessons from industry` である。 citeturn40view4turn37view0turn40view6turn36view7turn39view0turn37view3turn38view1
...
repo 引用候補は、`Petri 1962`、`van der Aalst 1998`、`van der Aalst et al. 2011` である。 citeturn36view0turn34view4turn17search18turn37view4turn37view5
...
repo 引用候補は、`OMG BPMN 2.0.2`、`White / Wohed et al.`、`Camunda BPMN docs` である。 citeturn34view0turn39view1turn39view2turn17search17
...
repo 引用候補は、`PMI Responsibility Assignment Matrix`、`Smith and Erwin RACI`、`PMBOK RAM references` である。 citeturn39view4turn20search0turn21search0
...
repo 引用候補は、`FIPS PUB 183`、`ICAM IDEF0`、`IDEF0 tool conformance docs` である。 citeturn34view5turn18search8
...
repo 引用候補は、`Engeström 1987`、`Engeström 2001`、`Virkkunen and Newnham 2013 / Change Laboratory` である。 citeturn35view6turn18search9turn31search19turn31search12
...
repo 引用候補は、`Dietz 1999/2006/2024`、`Winograd and Flores 1987`、`Schoop 2001` である。 citeturn41search2turn34view6turn37view6turn37view7turn19search3turn41search13
...
repo 引用候補は、`von Bertalanffy 1968`、`Wiener 1948`、`Vensim / STELLA systems thinking docs` である。 citeturn41search15turn41search0turn18search3turn38view5turn39view5
...
README では、この点を「既存理論から着想を受けた新規命名」と明記するのが学術的に誠実である。 citeturn10view2turn36view1turn16search17
...
この結果から、`responsible` は、まず理解しやすさを改善する説明モデルとして価値を出し、その後に形式化を足す導入順が妥当だと考えられる。 citeturn36view4turn36view5turn17search11turn22search2
...
そのため、`responsible` のように semantic core を単純化し、BPMN を主として view 層に退避する方針は、経験研究の知見と整合する。 citeturn34view0turn17search17turn36view5
...
Schema evolution と projection lifecycle を明記する必要がある。 citeturn40view4turn36view7
...
`responsible` に `requires` と `ensures` を導入する際も、すべてを完全形式化するより、まず簡潔な事実記述、責任記述、代表的不変条件から始めるほうが現実的である。 citeturn35view1turn36view6turn37view2
...
これらは投影先または実行先としての downstream implementation として整理するのが正しい。 citeturn10view2turn38view2turn38view3turn38view1

## 比較表

次表は、repo の公開テキスト、各理論の原典、標準、公式資料、利用実態やモデリング品質に関する経験研究をもとに要約したものである。
理論の成熟度は、学術的成熟度と実務定着度の両方を勘案している。 citeturn42view0turn10view0turn10view1turn10view2turn35view0turn35view1turn34view3turn34view4turn34view0turn36view5
...
`Activity<Input,Output>`、`requires`、`ensures`、`effects`、責任境界、view projection、RBNF を一本の文章でつなぐ。 citeturn42view0turn10view0turn10view1turn10view2turn35view0turn35view1turn34view3
...
次の短い理論マッピング表を README に置くと、読者は「これは何に近いのか」をすぐに把握できる。 citeturn35view0turn35view1turn34view3turn34view4turn34view0
...
以下は starter set である。 citeturn35view0turn35view1turn34view3turn34view4turn34view0turn36view1turn36view0
...
内容は公開されている README と docs の語彙に沿っている。 citeturn42view0turn10view0turn10view1turn10view2
...
ここは literate spec として明文化したほうがよい。 citeturn10view2
...
ここは actor model、Event Sourcing、Design by Contract のどれに寄せるかを決める必要がある。 citeturn10view1turn40view4turn38view2
...
`responsible` が将来 event-based implementation adapter を持つなら、`ensures` の schema change と `Effect` の versioning policy を別文書で定義する必要がある。 citeturn36view7
...
これらを明示できれば、`responsible` は「BPMN 風の可視化ツール」ではなく、「責任境界付き Activity 合成の意味論を持つモデル」として位置づけやすくなる。 citeturn10view2turn34view4turn16search6turn16search17
