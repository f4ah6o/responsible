...
- **自分のモデルを表示** — ツールバーから任意の `responsible.v0` JSON ファイルを読み込める。構造検証が JSON パス付きで問題を報告し、フラットなモデルは自動的に合成ルートで包まれる。
...
ビューアにはサンプルプロセス(ソフトウェア開発、ドキュメント出版、AI エージェント実行、分岐・合流のある見積承認フロー)が同梱されている。自分のプロセスを表示するには:

1. `responsible.v0` の JSON モデルを書く。[`examples/order-fulfillment.json`](examples/order-fulfillment.json)(受注〜請求の 6 Activity)から始めるとよい。
...
バージョン `0.x` — API と JSON スキーマ(`responsible.v0`)はマイナーバージョン間で変わる可能性がある。現在の v0 はセマンティックコアのうち検証可能なサブセットを実装している: 不変条件 `INV-1`–`INV-6`、plain data としての `Effect`、DAG 射影。実行 API と逆射影 API は持たない。ループの意味論が定義されるまで、循環は拒否される。
